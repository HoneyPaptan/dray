...
**Sent bubble = dark surface in a light window, so tag inside it take dark mode's accent not this palette's.** Default light fill bubble with `--primary` and write near-white prose on it, and light palette's accents are mid-tone — so coloured word on it read as smudge or vanish. Three thing tried and dropped: flatten to `inherit` (every tag white prose, colour saying nothing), box each run on `--card`, box each on solid white — both box read as thing stuck in sentence. What work = light, saturated ink, no fill.

**And two hue *rotate*, which is the part lightness and chroma cannot do.** Session read on that fill because it sit 90° off it; mention at 245 and issue at 310 sat 5° and 60° off hue-250 blue and stay weak however bright drawn. So on this surface alone mention go cyan (205) and issue go pink (330). Cost, stated: mention = blue in composer, cyan in bubble, so kind stop being one colour across both surface — paid because run nobody can tell from sentence round it name no kind at all. Declared as `--accent-*` on bubble itself, so anything drawn there later read same set; branded bubble skip it, its fill being light.
...
**Bubble stop at twenty lines, "Show more" under it.** Pasted stack trace or whole file otherwise fill transcript and bury answer it asked for. Twenty = long enough typed prompt never collapse, only paste do. Clip **measured**, not counted off string — whether twenty line hold prompt depend how wide column is, and "Show more" expanding to same twenty line = control doing nothing. Nothing truncated: prompt = one thing in transcript reader wrote themselves. Button sit **inside** bubble, where it stay attached to text it open — under bubble it = loose word on transcript's own background, nothing saying which message it belong to. Muted and at interface size not chat size, which = what keep it from reading as last line reader wrote. **Queued bubble share it** ([ClampedBody](src/components/chat/ClampedBody.tsx)) — held prompt and delivered one = same words seconds apart, so clamp on one alone mean paste fill transcript whole queued period then suddenly collapse. **Focus open it**: control clipped past twentieth line stay in tab order, so tabbing reach something nobody can see; `inert` would take *visible* text off accessibility tree too, worse than stray focus stop.
