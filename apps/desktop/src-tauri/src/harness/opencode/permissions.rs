//! opencode's `session/request_permission` onto Dray's permission card.
//!
//! ACP's own shape, so this is fx's module with the ids renamed: the options
//! are the server's and go back as they came.
//!
...
pub fn pending_for(request: &PermissionRequest, rpc_id: i64) -> (PendingRequest, Vec<PermissionOption>) {
    ...
    let pending = PendingRequest {
        ...
        // opencode rebuilds nothing from the answer — it is the option alone.
        input: Value::Null,
        options,
...
fn build_options(request: &PermissionRequest) -> Vec<ResolvedOption> {
        ...
        .filter_map(|choice| {
            ...
            Some(ResolvedOption {
                option: PermissionOption {
                    id: format!("opencode-{}", choice.option_id),
    ...
    if !options.iter().any(|o| o.option.behavior == Deny) {
        options.push(ResolvedOption {
            option: PermissionOption {
                id: "opencode-cancelled".to_string(),
...
mod tests {
    use super::*;
    use crate::harness::opencode::parser::{PermissionChoice, ToolCallRef};
    ...
    fn the_captured_options_become_three_buttons() {
        ...
        assert_eq!(
            pending.options["opencode-allow_once"].decision.as_ref().unwrap(),
    ...
    fn a_request_with_no_refusal_gets_one() {
        ...
        assert_eq!(
            pending.options["opencode-cancelled"].decision.as_ref().unwrap(),
