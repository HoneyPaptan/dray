...
Independent pieces of work can run as separate Dray sessions, each on its own branch and worktree. `dray new` creates one. Read the `dray` skill before your first `dray` command; it is installed at `~/.claude/skills/dray/SKILL.md`. Install: `curl -fsSL https://www.drayhq.com/install.sh | sh`.
